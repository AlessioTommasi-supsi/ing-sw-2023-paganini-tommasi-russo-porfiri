                ...
                case CPU_CHOICE -> {
                    switch (model.getPlayerChoice().getChoice()) {
                        ...
                        case JOIN_GAME:
                            ...
                            //modifica della gui!
                            //this.viewGui.refresh_message("Il player: "+model.getMyShelfie().getGame(this.currentGameId).getPlayers().get(model.getMyShelfie().getGame(this.currentGameId).getPlayers().size()-1).getUsername()+" just joined the game!");
                            new Thread(){
                                @Override
                                public void run(){
                                    loginGui.getTitleLabel().setText("Player: "+model.getMyShelfie().getGame(currentGameId).getPlayers().get(model.getMyShelfie().getGame(currentGameId).getPlayers().size()-1).getUsername()+" just joined the game!");
